...
impl AStarPlanner {
    ...
    pub fn planning(&self, sx: f64, sy: f64, gx: f64, gy: f64) -> Option<(Vec<f64>, Vec<f64>)> {
        ...
        let start_grid_node = utils::grid_map_planner::Node::new(
            start_node.x,
            start_node.y,
            start_node.cost,
        ...
        while let Some(current_item) = open_set.pop() {
            ...
            if iteration % 100 == 0 {
                println!(
                    "Iteration: {}, Open set size: {}, Closed set size: {}",
                    iteration,
                    open_set.len(),
                    closed_set.len()
                );
            ...
            let current_grid_node = utils::grid_map_planner::Node::new(
                current.x,
                current.y,
                current.cost,
            ...
            for (dx, dy, cost) in &self.motion {
                ...
                let new_grid_node = utils::grid_map_planner::Node::new(
                    new_node.x,
                    new_node.y,
                    new_node.cost,
        ...
        if SHOW_ANIMATION {
            ...
            fg.axes2d()
                .points(
                    &ox,
                    &oy,
                    &[Caption("Obstacles"), Color(gnuplot::RGBString("black"))],
                )
                .points(
                    &[sx],
                    &[sy],
                    &[Caption("Start"), Color(gnuplot::RGBString("green"))],
                )
                .points(
                    &[gx],
                    &[gy],
                    &[Caption("Goal"), Color(gnuplot::RGBString("blue"))],
                )
                .lines(
                    &rx,
                    &ry,
                    &[Caption("Path"), Color(gnuplot::RGBString("red"))],
                )
            ...
            // Save to file
            let output_path = "/Users/solz/Documents/rust/mouse_maze/output_data/img/path_planning/a_star_result.png";

...
from skimage.draw import (
    line as bresenham_line,
)  # Using a library for Bresenham's is cleaner
...
class AStarPathfinder:
    ...
    def _get_neighbors(self, node):
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                nx, ny = x + dx, y + dy
                if (
                    self.map._is_within_bounds(nx, ny)
                    and self.map.grid[ny, nx] < self.occupied_threshold
                ):
                    cost = sqrt(dx**2 + dy**2)
                    yield (nx, ny), cost
    ...
    def find_path(self, start_world, goal_world):
        ...
        if (
            not self.map._is_within_bounds(*start)
            or self.map.grid[start[1], start[0]] >= self.occupied_threshold
        ):
            print(f"Error: Start position {start_world} is invalid or occupied.")
            return None
        if (
            not self.map._is_within_bounds(*goal)
            or self.map.grid[goal[1], goal[0]] >= self.occupied_threshold
        ):
...
def main():
    ...
    try:
        """lidar_data_1758831298.json provides a good test case with clear start and goal positions."""
        with open(
            "/Users/solz/Documents/rust/mouse_maze/output_data/lidar/lidar_data_1761096364.json",
            "r",
        ) as f:
            lidar_data = json.load(f)
    except FileNotFoundError:
        print(
            "Error: lidar_data.json not found. Please place it in the same directory."
        )
        return
